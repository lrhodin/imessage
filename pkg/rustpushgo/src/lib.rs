...
#[uniffi::export(async_runtime = "tokio")]
impl Client {
    /// Diagnostic: fetch message records by record_name from messageManateeZone,
    /// decode each record's attributed_body as NSAttributedString, and log ALL
    /// attribute dictionary keys+values found in every range. Useful for
    /// discovering whether MMCS download metadata (e.g. __kIMFileTransferLocalUserInfoKey)
    /// is embedded in message records for attachments missing from attachmentManateeZone.
    pub async fn cloud_diag_message_attributed_keys(
        &self,
        record_names: Vec<String>,
    ) -> Result<(), WrappedError> {
        use rustpush::cloudkit::{FetchRecordOperation, FetchedRecords, CloudKitSession, NO_ASSETS};
        use rustpush::cloud_messages::{MESSAGES_SERVICE, CloudMessage};
        use rustpush::{coder_decode_flattened, NSAttributedString};

        info!("cloud_diag_message_attributed_keys: fetching {} message records from messageManateeZone", record_names.len());

        let cloud_messages = self.get_or_init_cloud_messages_client().await?;
        let container = cloud_messages.get_container().await.map_err(|e| WrappedError::GenericError {
            msg: format!("cloud_diag: get_container: {e}"),
        })?;
        ...
        let zone_key = container
            ...
            .await
            .map_err(|e| WrappedError::GenericError {
                msg: format!("cloud_diag: get_zone_encryption_config: {e}"),
            })?;

        let invoke = container
            .perform_operations(
                &CloudKitSession::new(),
                &FetchRecordOperation::many(&NO_ASSETS, &zone, &record_names),
                IsolationLevel::Operation,
            )
            .await
            .map_err(|e| WrappedError::GenericError {
                msg: format!("cloud_diag: perform_operations: {e}"),
            })?;

        let records = FetchedRecords::new(&invoke);

        for record_name in &record_names {
            // Try to decode with PCS key first; fall back to None key so we at least see unencrypted fields.
            let msg_opt: Option<CloudMessage> = {
                let rn = record_name.clone();
                let zk = zone_key.clone();
                let r1 = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    records.get_record::<CloudMessage>(&rn, Some(&zk))
                }));
                match r1 {
                    Ok(m) => Some(m),
                    Err(_) => {
                        warn!("cloud_diag {}: get_record with PCS key panicked, retrying without key", record_name);
                        let rn2 = record_name.clone();
                        let r2 = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            records.get_record::<CloudMessage>(&rn2, None)
                        }));
                        match r2 {
                            Ok(m) => Some(m),
                            Err(_) => {
                                warn!("cloud_diag {}: get_record without PCS key also panicked", record_name);
                                None
                            }
                        }
                    }
            ...
            match &msg.msg_proto.0.attributed_body {
                None => {
                    info!("cloud_diag {}: msg_proto.attributed_body = None", record_name);
                }
                Some(body_bytes) => {
                    info!("cloud_diag {}: attributed_body {} bytes", record_name, body_bytes.len());
                    let decode_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                        let flat = coder_decode_flattened(body_bytes);
                        if flat.is_empty() { return None; }
                        Some(NSAttributedString::decode(&flat[0]))
                    }));
                    match decode_result {
                        Ok(Some(attr_str)) => {
                            let text_preview = &attr_str.text[..attr_str.text.len().min(80)];
                            info!("cloud_diag {}: NSAttributedString text={:?} ranges={}", record_name, text_preview, attr_str.ranges.len());
                            for (i, (_len, dict)) in attr_str.ranges.iter().enumerate() {
                                let keys: Vec<&str> = dict.0.keys().map(|k| k.as_str()).collect();
                                info!("cloud_diag {}: range[{}] keys={:?}", record_name, i, keys);
                                for (key, val) in &dict.0 {
                                    // Log value type + summary (truncated for large values)
                                    let summary = format!("{:?}", val);
                                    let summary = if summary.len() > 200 { format!("{}…", &summary[..200]) } else { summary };
                                    info!("cloud_diag {}: range[{}] key={:?} val={}", record_name, i, key, summary);
                                }
                            }
                        }
                        Ok(None) => {
                            info!("cloud_diag {}: attributed_body present but coder_decode_flattened returned empty", record_name);
                        }
                        Err(_) => {
                            warn!("cloud_diag {}: NSAttributedString decode panicked", record_name);
                        }
                    }
                }
            }
        ...
        info!("cloud_diag_message_attributed_keys: done");
        Ok(())
    }
}
